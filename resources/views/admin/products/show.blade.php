...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
            ...
            <p class="text-2xl font-bold {{ $displayStock <= $product->low_stock_alert ? 'text-red-600' : 'text-green-600' }} mt-1">
                {{ $displayStock }}
            </p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
            <p class="text-sm text-gray-500">Quantité vendue</p>
            <p class="text-2xl font-bold text-blue-600 mt-1">{{ $ordersData['total_quantity'] ?? 0 }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
            <p class="text-sm text-gray-500">Chiffre d'affaires</p>
            <p class="text-2xl font-bold text-purple-600 mt-1">{{ number_format($ordersData['total_revenue'] ?? 0, 2) }} DH</p>
        </div>
    </div>
        ...
        <div class="xl:col-span-2 space-y-6 min-w-0">
            ...
            <section class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="p-5 sm:p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-5">
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-wide text-gray-400">Catégorie</p>
                            <p class="mt-1 font-medium text-gray-900">{{ $product->category?->name ?? 'Sans catégorie' }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-wide text-gray-400">SKU</p>
                            <p class="mt-1 font-medium text-gray-900">{{ $product->sku ?: 'Non défini' }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-wide text-gray-400">Alerte stock faible</p>
                            <p class="mt-1 font-medium text-gray-900">{{ $product->low_stock_alert }} unités</p>
                        </div>
                    </div>
                    <div class="space-y-5">
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-wide text-gray-400">Créé le</p>
                            <p class="mt-1 text-gray-700">{{ $product->created_at->format('d/m/Y à H:i') }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-wide text-gray-400">Modifié le</p>
                            <p class="mt-1 text-gray-700">{{ $product->updated_at->format('d/m/Y à H:i') }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-wide text-gray-400">Période d'activité</p>
                            <p class="mt-1 text-gray-700">{{ $ordersData['activity_period'] ?? 'N/A' }}</p>
                        </div>
                    </div>
            ...
            @if($product->variants->isNotEmpty())
                <section class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    <div class="p-5 sm:p-6 border-b border-gray-100 flex items-center justify-between gap-3">
                        <div>
                            <h2 class="text-lg font-bold text-gray-900">Variantes</h2>
                            <p class="text-sm text-gray-500">Prix, stock et options disponibles</p>
                        </div>
                        <span class="text-sm font-semibold text-purple-700 bg-purple-50 px-3 py-1 rounded-full">
                            {{ $activeVariants->count() }} actives
                        </span>
                    </div>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach($product->variants as $variant)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-5 py-4">
                                            <div class="font-medium text-gray-900">{{ $variant->option_label ?: 'Variante' }}</div>
                                            @if($variant->unit)
                                                <div class="text-xs text-gray-500 mt-1">Unité : {{ $variant->unit }}</div>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-5 py-4">
                                            <span class="font-semibold {{ $variant->stock_quantity <= $product->low_stock_alert ? 'text-red-600' : 'text-green-600' }}">
                                                {{ $variant->stock_quantity }}
                                            </span>
                                        </td>
                                        <td class="px-5 py-4">
                                            <span class="px-2.5 py-1 rounded-full text-xs font-semibold {{ $variant->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                                                {{ $variant->is_active ? 'Active' : 'Inactive' }}
                                            </span>
                                            @if($variant->is_default)
                                                <span class="ml-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-100 text-amber-700">
                                                    Défaut
                                                </span>
                                            @endif
                                        </td>
            ...
            <section class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div class="p-5 sm:p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">{{ $ordersData['total_orders'] ?? 0 }} commande(s) non annulée(s)</p>
                    </div>
                    <div class="text-sm text-gray-500">
                        Annulation : {{ $ordersData['cancellation_rate'] ?? 0 }}%
                    </div>
                </div>
                
                @if(isset($ordersData['recent_orders']) && $ordersData['recent_orders']->isNotEmpty())
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Commande</th>
                                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Variante</th>
                                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Qté</th>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach($ordersData['recent_orders'] as $item)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-5 py-4">
                                            <a class="font-semibold text-blue-600 hover:text-blue-800" href="{{ route('admin.orders.show', $item->order) }}">
                                                {{ $item->order?->order_number }}
                                            </a>
                                            <div class="text-xs text-gray-500 mt-1">{{ $item->created_at->format('d/m/Y H:i') }}</div>
                                        </td>
                                        <td class="px-5 py-4 text-sm text-gray-600">
                                            {{ $item->variant_snapshot ? implode(' / ', array_values($item->variant_snapshot)) : 'Produit standard' }}
                                        </td>
                                        <td class="px-5 py-4 font-semibold">{{ $item->quantity }}</td>
                                        <td class="px-5 py-4 font-semibold">{{ number_format($item->subtotal, 2) }} DH</td>
                                        <td class="px-5 py-4">
                                            <span class="px-2.5 py-1 rounded-full text-xs font-semibold {{ $item->order?->status_color ?? 'bg-gray-100 text-gray-700' }}">
                                                {{ $item->order?->status_label ?? 'N/A' }}
                                            </span>
                                        </td>
                                    </tr>
                                @endforeach
        ...
        <aside class="space-y-6 min-w-0">
            ...
            <section class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="font-bold text-gray-900">Galerie</h2>
                    <span class="text-sm text-gray-500">{{ $product->images->count() }}/10</span>
                </div>
            ...
            <section class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-semibold">{{ $ordersData['total_orders'] ?? 0 }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500">Moyenne / commande</span>
                        <span class="font-semibold">{{ $ordersData['avg_quantity_per_order'] ?? 0 }} unités</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500">Valeur moyenne</span>
                        <span class="font-semibold">{{ number_format($ordersData['average_order_value'] ?? 0, 2) }} DH</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500">Commandes annulées</span>
                        <span class="font-semibold text-red-600">{{ $ordersData['cancelled_orders_count'] ?? 0 }}</span>
                    </div>
                </div>
            </section>
            ...
            <form method="POST" action="{{ route('admin.products.destroy', $product) }}" onsubmit="return confirm('Supprimer définitivement ce produit ?')">
                @csrf
                @method('DELETE')
                <button type="submit" class="w-full inline-flex items-center justify-center px-4 py-3 rounded-xl border border-red-200 bg-red-50 text-red-700 font-semibold hover:bg-red-100 transition">
